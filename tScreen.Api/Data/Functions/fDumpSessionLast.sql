...
GO
IF OBJECT_ID('dbo.fDumpSessionLast') IS NOT NULL
BEGIN
    DROP FUNCTION dbo.fDumpSessionLast
END
GO

CREATE FUNCTION [dbo].[fDumpSessionLast] ()
    RETURNS TABLE AS
        RETURN
        WITH SessionDump AS (SELECT S.Id                                             AS [SessionId],
                             ...
                             WHERE S.Id = (SELECT TOP 1 Id FROM TweenScreenApp.Session ORDER BY CreatedAt DESC))
        SELECT SD.SessionId,
               SD.QuestionId,
               SD.AnswerId,
               SD.Category,
